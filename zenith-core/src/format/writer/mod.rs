...
//! - this module root holds the public entry point and the `zenith`/`project`/
//!   `assets`/`libraries`/`imports`/`components`/`masters`/`sections`/`provenance`/
//!   `variants`/`recipes`/`actions` orchestration;
//! - [`primitives`] holds shared format helpers (`indent`, `fmt_f64`,
...
use crate::ast::{
    ActionDef, AssetBlock, AssetDecl, BrandContract, ComponentDef, DiagnosticPolicy, Document,
    ImportDecl, LibraryDef, MasterDef, PolicyVerb, Project, ProvenanceDef, RecipeDef, RecipeParam,
    SectionDef, VariantDef,
};
...
pub(in crate::format::writer) use primitives::*;
