...
use crate::ast::node::{CodeNode, EllipseNode, LineNode, PatternNode, RectNode};
...
pub(in crate::validate::check) fn check_pattern(
    ...
    // Visual properties (token refs collected for token-usage checks). No
    // pattern-specific semantic checks yet (kind/spacing/count validation is not yet implemented).
    check_visual_prop(
        &p.id,
        "fill",
        p.fill.as_ref(),
