...
- **Use native icon components for real-world diagram objects.** Do not represent devices,
  ...
  Icons materialize as editable native `path` components; restyle them with instance overrides
  such as `override ref="icon.0" stroke=(token)"..." stroke-width=(token)"..."`.
...
| Architecture/product diagram icons — the full Lucide set (~1745) ships embedded; search rather than guess, and narrow with `--category` / `--kind` / `--limit` | `zenith library search <term>` · `zenith library show @zenith/icons-lucide#<icon>` · `zenith library add @zenith/icons-lucide#<icon> --into <file> --page <id> --at X,Y` |
